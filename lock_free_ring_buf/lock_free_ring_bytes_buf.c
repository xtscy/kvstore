...
//* 预留空间再拷贝
uint8_t LK_RB_Write_String(lock_free_ring_buffer *rb_handle, uint8_t *input_addr, uint32_t write_Length)
{
    if (write_Length == 0) {
        return RING_BUFFER_SUCCESS ;
    }
...
uint8_t LK_RB_Read_String(lock_free_ring_buffer *rb_handle, uint8_t *output_addr, uint32_t read_Length) {
