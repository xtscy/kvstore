...
static btree_node_t* create_node(bool is_leaf, int t) {
    ...
    node->keys = (bkey_t*)malloc(sizeof(bkey_t) * (2 * t - 1));
    if (!node->keys) {
        printf("malloc keys failed\n");
        exit(-1);
    }
    memset(node->keys, 0, sizeof(bkey_t) * (2 * t - 1));
...
btree_t* btree_create(int t) {
    ...
    btree->max_key = 2 * t - 1;
    btree->min_key = t - 1;
    if (pthread_rwlock_init(&btree->rwlock, NULL) != 0) {
        perror("pthread rwlock error");
        exit(-13);
        return NULL;
    }
    return btree;
...
bool btree_contains(btree_t* tree, bkey_t key) {
    ...
    while (current) {
        int i = 0;
        while (i < current->num_keys && /*key > current->keys[i]*/ strcmp(key.key, current->keys[i].key) > 0) i++;
        if (i < current->num_keys && strcmp(key.key, current->keys[i].key) == 0) {
            pthread_rwlock_unlock(&tree->rwlock);
            return true;
        }
...
// 没有重复键
static int find_key_idx(btree_node_t* node, bkey_t key) {
...
static bkey_t get_precursor(btree_node_t* node, int idx) {
...
// 拿到后继的最小值节点
static bkey_t get_successor(btree_node_t* node, int idx) {
...
static void borrow_from_left(btree_node_t* parent, int idx) {
    ...
    // 这里必然有位置，不会有溢出问题
    bkey_t p_key = parent->keys[idx - 1];
    // move key
    ...
    bkey_t left_key = left_node->keys[left_node->num_keys - 1];
...
static void remove_node(btree_node_t* node, bkey_t key, int t, fixed_size_pool_t *pool) {
    ...
    while (true) {
        ...
        if (c_idx < current->num_keys) {
            ...
            if (strcmp(current->keys[c_idx].key, key.key) == 0) {
                if (current->is_leaf) {
                    // 直接删除
                    fixed_pool_free(pool, current->keys[c_idx].data_ptrs);
                    ...
                    if (left_child->num_keys >= t) {
                        // 可以使用前驱替换，前驱有足够的键个数
                        fixed_pool_free(pool, current->keys[c_idx].data_ptrs);
                        bkey_t p_val = get_precursor(current, c_idx);
                        ...
                        fixed_pool_free(pool, current->keys[c_idx].data_ptrs);
                        bkey_t s_val = get_successor(current, c_idx);
...
bool btree_remove(btree_t* tree, bkey_t key, fixed_size_pool_t *pool) {
    ...
    remove_node(root, key, tree->t, pool);
    // 根节点键个数为0，降低树高度
