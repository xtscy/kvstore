...
int Process_Protocal(connection_t *c)
{
    ...
    while (1)
    {
        ...
        // } else {
        // 拿到当前所处的栈帧
        if (c->parser_stack.frames.state == STATE_INIT) {
        ...
        // }
                ...
                if (c->parser_stack.array.current_state == STATE_BULK_READING_LENGTH) {
                    ...
                    if (temp_c == '\n') {
                        ...
                        if (c->parser_stack.array.current_state == STATE_BULK_READING_LENGTH_LF) {
                            ...
                            // 判断当前写入的总数是否是预期值，如果是说明当前数组全部数据读取完毕   
                            // 上面完成了把Block写入到数组中的任务,这里进行状态更新
                            c->parser_stack.array.total_count++;
                            if (c->parser_stack.array.total_count == c->parser_stack.array.expected_count) {
                                // 说明当前元素全部读取完，那么去创建请求task_deli_t放入线程队列
                                // 这里是在读取批量字符串,说明读取完了最后一个批量字符串
                                // 这里把外部状态设置为完成，然后内层的array的状态在初始状态时进行初始化
                                c->parser_stack.frames.state = STATE_ARRAY_COMPLETE;
                            } else if (c->parser_stack.array.total_count < c->parser_stack.array.expected_count) {
                                // 继续判断是否需要继续读取元素
                                // 当前还没有读取完数据，那么继续去读取数据
                                c->parser_stack.array.current_state = STATE_READING_TYPE;
                            } else {
                                printf("total_count > expected_count\n");
                                abort();
                            }
                        }
                    } else {
            ...
            if (c->parser_stack.array.mode == MODE_INLINE_ONLY) {
                ...
                // 定义task结构，然后放入任务队列中
                // 先写安全判断,个数在读取长度时保证
                // if (c->parser_stack.array.total_count == 0) {

                // }
                // 这里用第一个字符串来判断是哪一个命令,先判断长度再判断值
                if (c->parser_stack.array.inline_elements[0].size == 3) {
                    if (memcmp(c->parser_stack.array.inline_elements[0].ptr, "set", 3) == 0) {
                    ...
                    } else if (memcmp(c->parser_stack.array.inline_elements[0].ptr, "get", 3) == 0) {
            ...
            while (1) {
                ...
                // g_thread_pool.workers[worker_idx]
                    ...
                    if (LK_RB_Write_Block(&g_thread_pool.workers[worker_idx].queue, &task, 1) == RING_BUFFER_SUCCESS) {
            ...
            // 当前数组的读取完成并且放入了任务队列中
            // 这里状态重置为STATE_INIT,重新去读取类型，并且在读取类型时如果是array类型，则初始化为0把整个array
            c->parser_stack.frames.state = STATE_INIT;
            // 如果动态开辟的则释放内存
            if (c->parser_stack.array.mode == MODE_HYBRID) {
                // free(c->parser_stack.array.dynamic.elements);
                // 动态需要释放资源，使用对应的内存释放策略
            }
