...
uint8_t RB_Write_Byte(ring_buffer *rb_handle, uint8_t data)
{
    ...
    //* 这里并没有判断下一个位置是否是有效位，是因为用了length和max_length比较
    //* 每次写的时候都会判断，如果不等说明当前尾指针指向的位置一定可写
    //*因为不可写的情况只有一种，就是首位相邻，如果没有，那么头指针以前的都可写，这里判断length等价判断是否相邻
    //* 因为数据的起点永远是头指针
 ...
 * \brief 从缓冲区头指针读取一个字节
 * \param[out] rb_handle: 缓冲区结构体句柄
 * \param[out] output_addr: 保存读取数据的缓冲区，由外部传入
 * \return 返回读取状态
...
uint8_t RB_Write_String(ring_buffer *rb_handle, uint8_t *input_addr, uint32_t write_Length)
{
    ...
    else
    {
        ...
        if((rb_handle->max_Length - rb_handle->tail) < write_Length)
        {
            ...
            rb_handle->tail = write_size_b ;//重新定位尾指针位置,指向当前存储数据位置的下一个位置，即直接指向下一个要写入的位置
        }
